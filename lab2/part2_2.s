...
# change .word to .byte for result and Grades labels
# change all lw or like sw to byte equivalent
# change index-ing to 1 for Grades (bytes = 1)
...
loop:
	...
	addi t1, t1, 1 # next Grades BYTE (1 byte)
	j loop

missing: 
	li s3, -1 # s3 <- -1
	la t4, result # t4 <- address of result
	sb s3, (t4) # contents of s3 go into memory address at t4; sb instead of sw
	j iloop # iloop label is already defined above (don't need to put iloop: j iloop)


found: 
	lb s3, (t1) # dereference t1 and store the contents at t1 memory address inside s3 (grade that matched student number) 
	la t4, result 
	sb s3, (t4) # sb instead of sw
	j iloop 
